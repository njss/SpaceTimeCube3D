...
(function() {
  ...
  d3.layout.force = function() {
   ...
   /* var newTopLeftX = 0.0;
    ...
    var first = true;*/
    function dragmove(d) {   
	   // alert('d3_identity: ' + d3_identity)
	  // alert( d3.behavior.drag().origin(d3_identity));
	    
	    touchMoved = true;
	    
        /*var deg = $("#deg").html()/180*3.1415926535;
        
        var x = 500.0;
        var y = 350.0;           
        
        var incX = x + Math.cos(-deg)*(d3.event.x-x)-Math.sin(-deg)*(d3.event.y-y) - newTopLeftX;
        var incY = y + Math.sin(-deg)*(d3.event.x-x)+Math.cos(-deg)*(d3.event.y-y) - newTopLeftY;  
       
        //$("#log").prepend( topLeft[0] + " " + topLeft[1] + "<br>");
        //$("#log").prepend( d3.event.x + " " + d3.event.y + "<br>");
        $("#log").prepend( incX + " " + incY + "<br>");
        //$("#log").prepend( newTopLeftX + " " + newTopLeftY + "<br>");

        newTopLeftX = newTopLeftX + incX;
        newTopLeftY = newTopLeftY + incY;
        ...
        else if(incX<50 && incY<50)
        {
                d.px = d.px + incX, d.py = d.py + incY;
        }
        */
  ...
  function d3_layout_forceDragend(d) {
	 ...
	 if (!touchMoved){
		...
		if (!isInValidWait)
			setTimeout(inValidateTouches,100);
	}
	
        //force.friction(.9);
        
	    if ((d.y > -50) && (d.y < 80) && (d.x > 0) && (d.x < 200)) 
	    {
            if(d.name == 'Population, total')
            {
                indicatorId = 'POPL';
            }
            if(d.name == 'Cereal production (metric tons)')
            {
                indicatorId = 'CERL';
            }
            if(d.name == 'Crop production index (2004-2006 = 100)')
            {
                indicatorId = 'CRPI';
            }
            if(d.name == 'Food production index (2004-2006 = 100)')
            {
                indicatorId = 'FOPI';
            }
            if(d.name == 'CO2 emissions (kt)')
            {
                indicatorId = 'CO2E';
            }
            if(d.name == 'CO2 emissions (metric tons per capita)')
            {
                indicatorId = 'CO2P';
            }
            if(d.name == 'CO2 intensity (kg per kg of oil equivalent energy use)')
            {
                indicatorId = 'CO2I';
            }
            if(d.name == 'PM10, country level (micrograms per cubic meter)')
            {
                indicatorId = 'PM10';
            }
            if(d.name == 'Organic water pollutant (BOD) emissions (kg per day)')
            {
                indicatorId = 'WBOD';
            }
            if(d.name == 'Immigration in, female')
            {
                indicatorId = 'IMIF';
            }
            if(d.name == 'Immigration out, female')
            {
                indicatorId = 'IMOF';
            }
            if(d.name == 'Immigration in, male')
            {
                indicatorId = 'IMIM';
            }
            if(d.name == 'Immigration out, male')
            {
                indicatorId = 'IMOM';
            }
            if(d.name == 'Immigration in, total')
            {
                indicatorId = 'IMIT';
            }
            if(d.name == 'Immigration out, total')
            {
                indicatorId = 'IMOT';
            }
            if(d.name == 'Country average temperature')
            {
                indicatorId = 'TAVG';
            }
            if(d.name == 'Country temperature maximum')
            {
                indicatorId = 'TMAX';
            }
            if(d.name == 'Country temperature minimum')
            {
                indicatorId = 'TMIN';
            }
  ...
  function inValidateTouches(){
	if (countTouchPos>0) {
		if (countTouchPos > lastTouchCount){
			lastTouchCount = countTouchPos;
			isInValidWait = true;
			setTimeout(inValidateTouches,150);
		} else {
			onTap(countTouchPos);
